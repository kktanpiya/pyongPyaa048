...
{
	[Activity (Label = "My Soal", ConfigurationChanges = ConfigChanges.ScreenSize | ConfigChanges.Orientation, ScreenOrientation = ScreenOrientation.Portrait)]
	public class MySoal_MainActivityDW : ActionBarActivity
	{
		...
		List<MySoal_LeftMenuListItemsDW> MenuListItem;
		List<MySoal_LeftMenuListItemsDW> ExMenuListItem;
		...
		protected override void OnCreate (Bundle bundle)
		{
			...
			mDrawerLayout = FindViewById<DrawerLayout> (Resource.Id.mySoal_Drawer);
			...
			leftListView = (LinearLayout)FindViewById (Resource.Id.mySoal_leftListView);
			lvMMLListed = (ListView)FindViewById (Resource.Id.lvMSMMLListed);
			lvMMLListedEx2 = (ListView)FindViewById (Resource.Id.lvMSMMLListedEx2);
			tvMMLUserEmail = (TextView)FindViewById (Resource.Id.tvMSMMLUserEmail);

			pager = FindViewById <ViewPager> (Resource.Id.pager);
			pager.Adapter = adapter;

			//due to policy changes, TAB IS DISABLED!
			/*
			...
			*/
			...
			ImageView ivMMLShowSecondMenu = (ImageView)FindViewById (Resource.Id.ivMSMMLShowSecondMenu);
			LinearLayout llMMLShowSecondMenu = (LinearLayout)FindViewById (Resource.Id.llMSMMLShowSecondMenu);
			...
			MenuListItem = new List<MySoal_LeftMenuListItemsDW> ();

			MenuListItem.Add (new MySoal_LeftMenuListItemsDW(){ 
			...
			*/
			...
			lvMMLListed.Adapter = new MySoal_MainActivityAdaptersDW (this, MenuListItem);
			lvMMLListed.ItemClick += menuItemClicked;

			ExMenuListItem = new List<MySoal_LeftMenuListItemsDW> ();

			//ExMenuListItem.Add (new LeftMenuListItems(){ 
			//	LeftMenuDrawerLabel = "Tetapan",
			//	LeftMenuDrawerIconID = Resource.Drawable.ic_gear_grey });

			ExMenuListItem.Add (new MySoal_LeftMenuListItemsDW(){ 
				LeftMenuDrawerLabel = "Berkenaan PI1M",
				LeftMenuDrawerIconID = Resource.Drawable.ic_about_48dp });

			lvMMLListedEx2.Adapter = new MySoal_MainActivityAdaptersDW (this, ExMenuListItem);
			...
			llMMLLogout = (LinearLayout)FindViewById (Resource.Id.llMSMMLLogout);
	...
	public class MySoal_MainActivityAdaptersDW : BaseAdapter<MySoal_LeftMenuListItemsDW>
	{
		List<MySoal_LeftMenuListItemsDW> dwListItems;
		Activity context;
		public MySoal_MainActivityAdaptersDW(Activity context, List<MySoal_LeftMenuListItemsDW> dwListItems)
		{
		...
		public override MySoal_LeftMenuListItemsDW this[int position]
		{
	...
	public class MySoal_LeftMenuListItemsDW
	{
		public MySoal_LeftMenuListItemsDW ()
		{
		}
