...
{
	public class List_Fragment_latestProd : Android.Support.V4.App.Fragment
	{
		Activity context;
		private RecyclerView mRecyclerView;
		RecyclerView.LayoutManager mLayoutManager;
		private LatestProd_RecyclerViewAdapter recyclerAdapter;

		private static List<MyShop_WebService.LatestProd_Datum> prodList = new List<MyShop_WebService.LatestProd_Datum>();

		int currentPage;
		int lastPage;
		static int  totalItem;

		public List_Fragment_latestProd (Activity Context)
		{
		...
		public override void OnActivityCreated(Bundle savedInstanceState)
		{
			...
			if (mRecyclerView != null) {
				...
				var onScrollListener = new LatestProd_RecyclerViewAdapter.ViewOnScrollListener (layoutManager);
		...
		{
			var jsonString = MyShop_WebService.GetJsonLatestProduct (page);
			var ProdData = JsonConvert.DeserializeObject<MyShop_WebService.Root_LatestProd> (jsonString);
			lastPage = ProdData.last_page;
			totalItem = ProdData.total;
			foreach (var tempData in ProdData.data) {
				prodList.Add (new MyShop_WebService.LatestProd_Datum () {
			...
			context.RunOnUiThread (() => {
				if (currentPage == 1) {
					...
					recyclerAdapter = new LatestProd_RecyclerViewAdapter (Activity);
		...
		public class LatestProd_RecyclerViewAdapter : RecyclerView.Adapter
		{
			TypedValue typedValue = new TypedValue();
			int background;
			Android.App.Activity parent;

			public class LatestProd_ViewHolder : RecyclerView.ViewHolder
			{
				...
				public LatestProd_ViewHolder (View view) : base (view)
				{
					View =  view;
			...
			public LatestProd_RecyclerViewAdapter (Android.App.Activity context)
			{
				parent = context;
			...
			public override RecyclerView.ViewHolder OnCreateViewHolder (ViewGroup parent, int viewType)
			{
				...
				if (viewType == VIEW_TYPES.NORMAL || viewType == VIEW_TYPES.ENDOFLIST) {
					...
					return new LatestProd_ViewHolder (view);
				}
				else {
