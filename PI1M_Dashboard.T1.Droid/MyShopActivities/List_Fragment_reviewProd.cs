...
{
	public class List_Fragment_reviewProd : Android.Support.V4.App.Fragment
	{
		Activity context;
		private RecyclerView mRecyclerView;
		ProgressDialog progressDialog;
		RecyclerView.LayoutManager mLayoutManager;
		private ReviewProd_RecyclerViewAdapter recyclerAdapter;

		private static List<MyShop_WebService.ReviewProd_Datum> prodList = new List<MyShop_WebService.ReviewProd_Datum>();
		...
		public List_Fragment_reviewProd (Activity Context)
		{
		...
		{
			var jsonString = MyShop_WebService.GetJsonReviewProduct (page);
			var ProdData = JsonConvert.DeserializeObject<MyShop_WebService.Root_ReviewProd> (jsonString);
			lastPage = ProdData.last_page;
			totalItem = ProdData.total;
			foreach (var tempData in ProdData.data) {
				prodList.Add (new MyShop_WebService.ReviewProd_Datum () {
			...
			context.RunOnUiThread (() => {
				if (currentPage == 1) {
					...
					recyclerAdapter = new ReviewProd_RecyclerViewAdapter (Activity);
		...
		public class ReviewProd_RecyclerViewAdapter : RecyclerView.Adapter
		{
			TypedValue typedValue = new TypedValue();
			int background;
			Android.App.Activity parent;

			public class ReviewProd_ViewHolder : RecyclerView.ViewHolder
			{
				...
				public ReviewProd_ViewHolder (View view) : base (view)
				{
					View =  view;
			...
			public ReviewProd_RecyclerViewAdapter (Android.App.Activity context)
			{
				parent = context;
			...
			public override RecyclerView.ViewHolder OnCreateViewHolder (ViewGroup parent, int viewType)
			{
				...
				if (viewType == VIEW_TYPES.NORMAL || viewType == VIEW_TYPES.ENDOFLIST) {
					...
					return new ReviewProd_ViewHolder (view);
				}
				else {
